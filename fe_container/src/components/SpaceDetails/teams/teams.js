...
import TeamsUserList from './teams-user-list'
...
import TeamPlaceholderIcon from '../../../assets/img/icons/team-placeholder-icon.svg'
...
const Teams = () => {
  ...
  const [hasDeleteTeamModal, setHasDeleteTeamModal] = useState(false)
  ...
  const refsById = useMemo(() => {
    const refs = {}
    teams?.forEach((item) => {
      refs[item.team_id] = createRef(null)
    })
    return refs
  }, [teams])
  ...
  const getUsersData = async (arg, container) => {
    const res = await apiHelper({
      ...
      subUrl: process.env.TEAMS_LIST_USERS,
      value: arg,
      spaceId,
    })
  ...
  const getTeamsList = async () => {
    ...
    const res = await apiHelper({
      baseUrl: process.env.BLOCK_ENV_URL_API_BASE_URL,
      subUrl: process.env.LIST_TEAMS_URL,
      value: filterDataStructure(),
      spaceId,
    })
    setTeams(res ? res.data : null)
  ...
  useEffect(async () => {
    const res =
      usersFilterData?.team_id &&
      (await apiHelper({
        baseUrl: process.env.BLOCK_ENV_URL_API_BASE_URL,
        subUrl: process.env.TEAMS_LIST_USERS_URL,
  ...
  const onCreateNewTeam = async (appName) => {
    await apiHelper({
      baseUrl: process.env.BLOCK_ENV_URL_API_BASE_URL,
      subUrl: process.env.CREATE_TEAM_URL,
      value: {
        name: appName,
        description: '',
        space_id: spaceId,
  ...
  const onDeleteTeam = async () => {
    await apiHelper({
      baseUrl: process.env.BLOCK_ENV_URL_API_BASE_URL,
      subUrl: process.env.DELETE_TEAM_URL,
      value: {
    ...
    setHasDeleteTeamModal(false)
  }

  const onDeleteUser = async () => {
    await apiHelper({
      baseUrl: process.env.BLOCK_ENV_URL_API_BASE_URL,
      subUrl: process.env.TEAMS_DELETE_USER_URL,
      value: {
        team_id: selectedUser,
      },
      apiType: 'delete',
      spaceId,
    })
  ...
  return (
    <div className="float-left w-full">
            ...
            <div className="border-ab-gray-dark float-left flex w-full items-center justify-between border-b px-4 py-3">
              ...
              <div className="float-left flex items-center space-x-2">
                <div className="float-left flex items-center space-x-2">
                  <button
                    ...
                    className={`btn-default bg-ab-yellow text-ab-black disabled:bg-ab-disabled h-8 flex-shrink-0 rounded px-3 py-2 text-xs font-medium leading-tight transition-all hover:opacity-90 ${
                      accordianID === item.team_id ? '' : 'hidden'
                    }`}
                  >
                    Invite new member
                  </button>
                ...
                <div className="flex h-8 items-center">
                  <img
                    onClick={() =>
                      handleAccordian(item.team_id, refsById[item.team_id])
                    }
                    className={`flex-shrink-0 cursor-pointer transition-all duration-100 ${
                      accordianID === item.team_id && 'rotate-180'
                    }`}
                    src={ExpandMoreIcon}
                    alt=""
                  />
      ...
      {!loader && !teams && !searchText && (
        <EmptyMsg
          border={false}
          img={TeamPlaceholderIcon}
          text="You don’t have any teams"
          buttonText="Create your first team"
          buttonType="team"
          paddingY="10"
          marginY="2"
      ...
      {hasDeleteTeamModal && (
        <DeleteUserModal
          hasDeleteUserModal={hasDeleteTeamModal}
          handleDeleteUserModal={() => {
            setHasDeleteTeamModal(false)
            setSelectedTeam(null)
          }}
          handleSubmit={() => onDeleteTeam()}
          type="Team"
        />
      )}
      {hasDeleteUserModal && (
      ...
      {hasCreateNewModal && (
        <CreateNewModal
          ...
          handleClick={(teamName) => onCreateNewTeam(teamName)}
          type="Team"
      ...
      {hasAclModal && (
        <AclSidebarModal
          ...
          modalTitle={`${
            currentTeam?.name ? `${currentTeam?.name}'s` : 'Team'
          } Access Management`}
        >
          <Policies
            hasTabMenu
            tabMenus={['Permissions', 'Member', 'Entities']}
            getExistingPolicies={process.env.TEAMS_LIST_EXISTING_POL_GRP_SUBS}
            getPoliciesToAdd={process.env.TEAMS_LIST_TO_ADD_POL_GRP_SUBS}
            current={currentTeam}
            type="Team"
          />
        </AclSidebarModal>
