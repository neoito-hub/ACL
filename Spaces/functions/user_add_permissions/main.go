...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	// var deletedPolGrpIDs []string
	// var entityMappingsCreateData map[string]interface{}
	...
	tx := db.Begin()
	ohMapstring, _ := json.Marshal(payload)

	fmt.Printf("owner user check is   %v\n", string(ohMapstring))
 ...
 LEFT JOIN (select t.team_id, name from team_members tm 
	...
	from (select subs.* from ac_pol_grp_subs subs where subs.owner_space_id=@owner_space_id and subs.owner_user_id=@owner_user_id
	...
	ac_permissions acper inner join 
	 (select subs.* from ac_pol_grp_subs subs where subs.owner_space_id=@owner_space_id and subs.owner_user_id=@owner_user_id
...
as predefined_policy_groups  from 
	...
	handlerResp = common_services.BuildResponse(false, "Permissions for user updated successfully!", Response{Data: resp, Err: false, Msg: "Permission subscription for user updated successfully!"}, http.StatusOK)
	return handlerResp
}
