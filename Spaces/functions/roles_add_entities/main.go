...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	existingValuesMap["roleID"] = b.RoleID
	...
	select subs.polgrpsubs,etmap.etmappings from 
		(
			...
			where subs.role_id=@roleID and subs.owner_space_id=@spaceID
		 ) subs
		 left join (
	...
	subsValuesMap["roleID"] = b.RoleID
	mappingValuesMap["roleID"] = b.RoleID
	...
	existingMappingsDeleteQuery := `delete from pol_grp_subs_entity_mappings etmap using (select etmap.id as mapping_id from
		...
		and subs.role_id=@roleID and subs.owner_space_id=@spaceID
	...
	emptyPolicyGroupsDeleteQuery := `delete from ac_pol_grp_subs subs 
		using (select subs.id,count(etmap.id) as mapping_count from ac_pol_grp_subs subs inner join 
			  ...
			  and subs.role_id=@roleID and pol_grp.entity_type=1 and subs.owner_space_id=@spaceID group by subs.id) selsubs
	...
	handlerResp = common_services.BuildResponse(false, "Entities for roles added successfully", Response{Data: Response{}, Err: false, Msg: "Entities for roles added successfully"}, http.StatusOK)
	return handlerResp
}
