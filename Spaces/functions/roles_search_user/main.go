...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	// TODO
	db.Raw(`SELECT u.user_id,u.user_name,u.email FROM users u left join member_roles mr on (mr.owner_user_id = u.user_id and mr.role_id=?) left join space_members s on s.owner_user_id = u.user_id and s.owner_space_id = (select owner_space_id from roles where id=?) left join (select iv.id, iv.email from invites iv inner join invite_details id on id.invite_id = iv.id and id.invited_role_id = ? where now()<iv.expires_at) i on i.email=u.email where mr.id is null and s.id is not null and i.id is null and (u.user_name ilike ? or u.email ilike ?)`, b.RoleID, b.RoleID, b.RoleID, searchParam, searchParam).Scan(&userDetails)
