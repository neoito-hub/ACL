...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
 ...
 LEFT JOIN (select t.team_id, name from team_members tm 
	...
	selected_user_pgs as (
		...
		WHERE  pgs.owner_space_id=@owner_space_id AND pgs.owner_user_id=@selected_user_id
	)
 ...
 LEFT JOIN (select t.team_id, name from team_members tm 
	...
	valuesMap["selected_user_id"] = b.UserID
	...
	handlerResp = common_services.BuildResponse(false, "Available permissions for user listed successfully!", Response{Data: resultData, Err: false, Msg: "Available permissions for user listed successfully!"}, http.StatusOK)
	return handlerResp
}
