...
func VerifyAndGetUser(w http.ResponseWriter, r *http.Request) (*ShieldUserData, error) {
	...
	if jsonErr != nil {
		return &ShieldUserData{}, jsonErr
	}

	if !responseObject.Success {
		fmt.Println(responseObject.Data)
		return &ShieldUserData{}, errors.New(responseObject.Data.(string))
	}
...
func GenerateNonParameterisedQuery(inputFilter *string, suffix string, optype string) {
	if len(*inputFilter) == 0 {
		*inputFilter += " where " + suffix
	} else {
		*inputFilter += " " + optype + " " + suffix
	}
