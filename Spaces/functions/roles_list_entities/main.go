...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	--getting already existing entity mappings
		existing_entity_mappings as (select subs.id as subs_id,etmap.owner_entity_id as entity_id,etmap.id as entity_mapping_id,polgrp.name as pol_grp_name,polgrp.id as pol_grp_id from (select * from ac_pol_grp_subs) subs 
			...
			inner join pol_grp_subs_entity_mappings etmap on etmap.pol_grp_subs_id=subs.id
			where subs.owner_space_id=@spaceID and subs.role_id=@roleID)
	...
	left join lateral (
		...
		where (ext.pol_grp_id is null and polgrp.is_predefined and polgrp.entity_types @> array_append(array[]::integer[],@typeID)) or (ext.pol_grp_id is not null))etmap on true
	`, policyGroupName, blocksFilterString, orderByString+limitString+offsetString)

	if err := db.Raw(query, valuesMap).Scan(&entityData.Entities).Error; err != nil {
		handlerResp = common_services.BuildErrorResponse(true, "Error getting entities for role", Response{}, http.StatusInternalServerError)
		return handlerResp
	...
	resp.Err = false
	resp.Msg = "Entities for the role retrieved successfully"

	handlerResp = common_services.BuildResponse(false, "Entities for the role retrieved successfully", resp, http.StatusOK)
