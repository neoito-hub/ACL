...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	for i, p := range insertData {
		...
		resp = append(resp, ResponseData{ID: insertData[i].ID, SpaceID: p.OwnerSpaceID, TeamID: p.OwnerTeamID, AcPolGrpID: p.AcPolGrpID})
	}
	...
	// RespondWithJSON(w, http.StatusOK, Response{Data: resp, Err: false, Msg: "Policy subscription for teams added successfully!"})

	handlerResp = common_services.BuildResponse(false, "Policy subscription for teams added successfully!", Response{Data: resp, Err: false, Msg: "Policy subscription for teams added successfully!"}, http.StatusOK)
	return handlerResp
