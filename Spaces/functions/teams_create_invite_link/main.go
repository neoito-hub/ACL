package teams_create_invite_link
...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	spaceTeamMap := make(map[string]map[string]bool)
	spaceMap := make(map[string][]string)

	for _, space := range b.Data {
		if _, ok := spaceTeamMap[space.SpaceID]; ok {

		} else {
			spaceTeamMap[space.SpaceID] = make(map[string]bool)
		...
		for _, team := range space.TeamIDs {
			...
			if _, ok := spaceTeamMap[space.SpaceID][team]; ok {

			} else {
				spaceTeamMap[space.SpaceID][team] = true
				spaceMap[space.SpaceID] = append(spaceMap[space.SpaceID], team)
	...
	for _, space := range spaceArray {
		...
		teamsString := strings.Join(spaceMap[space], "~")
