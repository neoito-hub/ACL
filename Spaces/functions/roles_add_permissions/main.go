...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
 ...
 LEFT JOIN (select t.team_id, name from team_members tm 
	...
	from (select subs.* from ac_pol_grp_subs subs where subs.owner_space_id=@owner_space_id and subs.role_id=@owner_role_id
	...
	ac_permissions acper inner join 
	 (select subs.* from ac_pol_grp_subs subs where subs.owner_space_id=@owner_space_id and subs.role_id=@owner_role_id
...
as predefined_policy_groups  from 
	...
	handlerResp = common_services.BuildResponse(false, "Permissions for the role updated successfully!", Response{Data: resp, Err: false, Msg: "Permission subscription for role added successfully!"}, http.StatusOK)
	return handlerResp
}
