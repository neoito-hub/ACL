...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	res := tx.Where("id = ?", b.ID).Unscoped().Delete(&models.MemberRole{})
	if res.Error != nil {
		tx.Rollback()
	...
	// 	Err: false,
	// 	Msg: "Member role removed successfully!",
	// })

	handlerResp = common_services.BuildResponse(false, "Member role removed successfully!", Response{
		Err: false,
		Msg: "Member role removed successfully!",
	}, http.StatusOK)
	return handlerResp
