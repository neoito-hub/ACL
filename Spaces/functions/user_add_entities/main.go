...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	existingValuesMap["userID"] = b.UserID
	existingValuesMap["spaceID"] = payload.SpaceID

	existingEntitiesQuery := `
		select subs.polgrpsubs,etmap.etmappings from 
		(
			select json_object_agg(subs.ac_pol_grp_id,subs.id) as polgrpsubs 
		
		 from ac_pol_grp_subs
		 subs where subs.owner_user_id=@userID and subs.owner_space_id=@spaceID
		 )subs left join
		 (select json_object_agg(etmap.owner_entity_id,etmap.polgrpids) as etmappings from 
		 (select etmap.owner_entity_id,string_agg(subs.ac_pol_grp_id,',') as polgrpids from
		 pol_grp_subs_entity_mappings etmap
		 inner join ac_pol_grp_subs subs on subs.id=etmap.pol_grp_subs_id
		  where subs.owner_user_id=@userID and subs.owner_space_id=@spaceID
		 group by etmap.owner_entity_id)etmap 
		  )etmap on true
		`
	...
	subsValuesMap["userID"] = b.UserID
	mappingValuesMap["userID"] = b.UserID
	...
	existingMappingsDeleteQuery := `delete from pol_grp_subs_entity_mappings etmap using (select etmap.id as mapping_id from
		...
		and subs.owner_user_id=@userID and subs.owner_space_id=@spaceID
	...
	emptyPolicyGroupsDeleteQuery := `delete from ac_pol_grp_subs subs 
		using (select subs.id,count(etmap.id) as mapping_count from ac_pol_grp_subs subs inner join 
			  ...
			  and subs.owner_user_id=@userID and pol_grp.entity_type=1 and subs.owner_space_id=@spaceID group by subs.id) selsubs
	...
	handlerResp = common_services.BuildResponse(false, "Entities for user added successfully", Response{Data: Response{}, Err: false, Msg: "Entities for user added successfully"}, http.StatusOK)
	return handlerResp
}
