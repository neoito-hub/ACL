...
func GenerateNonParameterisedQuery(inputFilter *string, suffix string, optype string, existingFilter string) {
...
func AttachToMainfilter(subfilter *string, mainfilter *string) {

	if len(*mainfilter) == 0 {
		*mainfilter = " where " + "(" + *subfilter + ") "
	} else {
		*mainfilter += " and " + "(" + *subfilter + ") "
	}
