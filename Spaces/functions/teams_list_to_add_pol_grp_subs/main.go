package teams_list_to_add_pol_grp_subs
...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	teamsQuery1 := `SELECT pg.id AS ac_pol_grp_id, pg.name, pg.description, pg.is_predefined, pg.created_at, pg.updated_at, pgs.id subs_id FROM ac_pol_grps pg LEFT JOIN (select * from ac_pol_grp_subs where permission_id is null ) pgs ON pgs.ac_pol_grp_id=pg.id AND pgs.owner_space_id = @owner_space_id AND pgs.owner_team_id = @owner_team_id `
	filterString = ` WHERE  (pg.owner_space_id=@owner_space_id OR coalesce(pg.owner_space_id, '') =  '')`

	countQuery := `SELECT COUNT(pg.id) as total_count FROM ac_pol_grps pg LEFT JOIN ac_pol_grp_subs pgs ON pgs.ac_pol_grp_id=pg.id AND pgs.owner_space_id = @owner_space_id AND pgs.owner_team_id = @owner_team_id`
	...
	teamsQuery1 = teamsQuery1 + filterString + " limit @limit offset @offset"
	...
	res := db.Raw(teamsQuery1, valuesMap).Scan(&policiesList)
	...
	// RespondWithJSON(w, http.StatusOK, Response{Data: resultData, Err: false, Msg: "Policy subscription for teams listed successfully!"})

	handlerResp = common_services.BuildResponse(false, "Policy subscription for teams listed successfully!", Response{Data: resultData, Err: false, Msg: "Policy subscription for teams listed successfully!"}, http.StatusOK)
	return handlerResp
}
