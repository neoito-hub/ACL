...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	nameCheckQuery := `select exists(select * from spaces where LOWER(name) = LOWER(?) limit 1);`
	res := db.Raw(nameCheckQuery, b.Name).Scan(&nameExists)
