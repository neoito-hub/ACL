...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	with pg_subs as (
		select pgs.id,pg.display_name,pgs.permission_id,pgs.role_id,pgs.owner_space_id,unnest(pg.entity_types) as entity_type from
		ac_pol_grp_subs pgs 
		INNER JOIN ac_pol_grps pg ON pg.id = pgs.ac_pol_grp_id
		INNER JOIN roles r on r.id = pgs.role_id
		WHERE  pgs.owner_space_id=@owner_space_id AND r.id=@owner_role_id
	)
...
SELECT acper.id as permission_id,acper.display_name as name, acper.description,json_agg(distinct jsonb_build_object('subs_id',pg_subs.id,'policy_group_name',pg_subs.display_name)) policy_groups, count(pg_subs.id) as pg_count,jsonb_agg(distinct jsonb_build_object('entity_id',et.entity_id,'entity_type',et.type,'label',et.label)) filter (where et.entity_id is not null) as entities,json_object_agg(distinct coalesce(pg_subs.entity_type,0),true) as entity_types FROM 
	...
	with pg_subs as (
		select pgs.id,pg.display_name,pgs.permission_id,pgs.owner_space_id,r.id from
		ac_pol_grp_subs pgs 
		INNER JOIN ac_pol_grps pg ON pg.id = pgs.ac_pol_grp_id
		INNER JOIN roles r on r.id = pgs.role_id
		WHERE  pgs.owner_space_id=@owner_space_id AND r.id=@owner_role_id
	...
	valuesMap["owner_role_id"] = b.RoleID
	...
	handlerResp = common_services.BuildResponse(false, "Existing permission subscription for role listed successfully!", Response{Data: resultData, Err: false, Msg: "Existing permission subscription for role listed successfully!"}, http.StatusOK)
	return handlerResp
}
