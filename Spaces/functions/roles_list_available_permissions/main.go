...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
 ...
 LEFT JOIN (select t.team_id, name from team_members tm 
	...
	selected_role_pgs as (
		...
		WHERE  pgs.owner_space_id=@owner_space_id AND r.id=@selected_role_id 
	)
	...
	(select acper.* from ac_permissions acper left join selected_role_pgs selpgs on selpgs.permission_id=acper.id where selpgs.permission_id is null )acper
 ...
 LEFT JOIN (select t.team_id, name from team_members tm 
	...
	valuesMap["selected_role_id"] = b.RoleID
	...
	handlerResp = common_services.BuildResponse(false, "Available permission for role listed successfully!", Response{Data: resultData, Err: false, Msg: "Available permission for role listed successfully!"}, http.StatusOK)
	return handlerResp
}
