...
func Handler(payload common_services.HandlerPayload) common_services.HandlerResponse {
	...
	teamsQuery1 := `SELECT pgs.id AS subs_id, pg.name, pg.description, pg.is_predefined, pgs.created_at, pgs.updated_at,acper.display_name as permission_name FROM (select * from ac_pol_grp_subs) pgs INNER JOIN ac_pol_grps pg ON pg.id = pgs.ac_pol_grp_id  left join ac_permissions acper on acper.id=pgs.permission_id`
	filterString = ` WHERE  pgs.owner_space_id=@owner_space_id AND owner_team_id=@owner_team_id`
	...
	// RespondWithJSON(w, http.StatusOK, Response{Data: resultData, Err: false, Msg: "Existing policy subscription for teams listed successfully!"})

	handlerResp = common_services.BuildResponse(false, "Existing policy subscription for teams listed successfully!", Response{Data: resultData, Err: false, Msg: "Existing policy subscription for teams listed successfully!"}, http.StatusOK)
	return handlerResp
}
